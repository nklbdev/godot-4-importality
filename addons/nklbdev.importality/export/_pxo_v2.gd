...
static func export(res_source_file_path: String, options: Dictionary) -> _Export.ExportResult:
	...
	for animation_index in animations_count:
		...
		if is_animation_default:
			...
			var animation_params_parsing_result := _Export._parse_animation_params(
				pxo_tag.name, _Export.AnimationOptions.Direction | _Export.AnimationOptions.RepeatCount,
	...
	var sprite_sheet_building_result := sprite_sheet_builder.build_sprite_sheet(unique_frames_images)
