		...
		_Models.SpriteSheetModel.Layout.PACKED:
			match options[_Options.ANIMATION_STRATEGY]:
				...
				AnimationStrategy.SPRITE_REGION_AND_OFFSET:
					...
					# sprite.region_filter_clip_enabled = options[_Options.ATLAS_TEXTURE_REGION_FILTER_CLIP_ENABLED]
					animation_player = _create_animation_player(export_result.animation_library, {
						".:offset": func(frame_model: _Models.FrameModel) -> Vector2:
							return Vector2( # spatial sprite offset (the Y-axis is Up-directed)
								frame_model.sprite.offset.x,
								sprite_size.y - frame_model.sptite.offset.y - \
								frame_model.sprite.region.size.y) + \
								# add center correction
								((frame_model.sprite.region.size - sprite_size) * 0.5
